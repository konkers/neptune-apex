...
async fn main() {
    ...
    let mut client = HttpClient::new(&TokioTcp, &StdDns);
    let mut apex = Apex::new(client, "192.168.3.36").unwrap();
    apex.auth("konkers", env!("PASSWORD")).await.unwrap();
}
