...
impl embedded_nal_async::Dns for StdDns {
    ...
    async fn get_host_by_name(
        ...
        for address in (host, 0).to_socket_addrs()? {
            match address {
                SocketAddr::V4(a)
                    if addr_type == AddrType::IPv4 || addr_type == AddrType::Either =>
                {
                    return Ok(IpAddr::V4(a.ip().octets().into()))
                }
                SocketAddr::V6(a)
                    if addr_type == AddrType::IPv6 || addr_type == AddrType::Either =>
                {
...
async fn main() {
    ...
    let Some(login) = option_env!("LOGIN") else {
        log::error!("No LOGIN environment variable");
        return;
    };

    let Some(password) = option_env!("PASSWORD") else {
        log::error!("No PASSWORD environment variable");
        return;
    };

    let client = HttpClient::new(&TokioTcp, &StdDns);
    let mut rx_buf = [0u8; 32768];
    let mut apex = Apex::new(
        client,
        "192.168.3.36",
        login,
        password,
        option_env!("SESSION_ID"),
    )
    .unwrap();

    let status = apex.status(&mut rx_buf).await.unwrap();
    log::info!("status: {status:#?}");
    let feed = Feed::None;
    apex.feed(&mut rx_buf, feed).await.unwrap();
    log::info!("set feed to {feed:?}");
}
